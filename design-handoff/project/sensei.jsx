/* global React */
/*
 * Sensei — friendly karate master, flat-vector cartoon.
 * Pose is parameterized so we can animate his expression with the lesson.
 */

const { useMemo } = React;

function Sensei({ mood = 'happy', talking = true }) {
  // Eye & mouth shapes change with mood. Kept tiny + warm.
  const eyes = useMemo(() => {
    switch (mood) {
      case 'cheer':  return { kind: 'closed-up' };
      case 'think':  return { kind: 'side' };
      case 'wow':    return { kind: 'wide' };
      default:       return { kind: 'open' };
    }
  }, [mood]);

  const renderEye = (cx, cy, mirror) => {
    if (eyes.kind === 'closed-up') {
      return (
        <path
  ...
  return (
    <svg viewBox="0 0 400 520" width="100%" height="100%" style={{ overflow: 'visible' }}>
      ...
      <g>
        {/* Pant base (peeking) */}
        <path d="M 105 470 Q 100 510 130 510 L 270 510 Q 300 510 295 470 L 295 430 L 105 430 Z"
              fill="#f0e1b4" stroke="#1f1712" strokeWidth="4" strokeLinejoin="round" />
        {/* Gi torso */}
        <path d="M 80 300 Q 70 280 95 270 L 305 270 Q 330 280 320 300 L 320 440 Q 320 460 300 460 L 100 460 Q 80 460 80 440 Z"
              fill="url(#gi)" stroke="#1f1712" strokeWidth="5" strokeLinejoin="round" />
        {/* Gi lapel left */}
      ...
      <g>
        {/* Right arm waving up */}
        <path d="M 305 295 Q 360 250 360 180 Q 360 145 325 145 Q 295 145 295 200 L 295 300 Z"
              fill="url(#gi)" stroke="#1f1712" strokeWidth="5" strokeLinejoin="round" />
        {/* Hand right (open palm) */}
        <g>
          <circle cx="335" cy="135" r="22" fill="url(#skin)" stroke="#1f1712" strokeWidth="4" />
        ...
        {/* Left arm at side */}
        <path d="M 95 295 Q 60 320 65 380 Q 68 420 92 416 Q 110 412 108 380 L 105 300 Z"
              fill="url(#gi)" stroke="#1f1712" strokeWidth="5" strokeLinejoin="round" />
        {/* Left hand */}
        <circle cx="80" cy="418" r="20" fill="url(#skin)" stroke="#1f1712" strokeWidth="4" />
      </g>

      {/* Head */}
      <g className={talking ? 'bob' : ''} style={{ transformOrigin: '200px 280px' }}>
        {/* Neck */}
        <rect x="180" y="240" width="40" height="40" fill="url(#skin)" stroke="#1f1712" strokeWidth="4" />
        {/* Face */}
        <ellipse cx="200" cy="200" rx="78" ry="86" fill="url(#skin)" stroke="#1f1712" strokeWidth="5" />
        {/* Cheeks */}
        <ellipse cx="155" cy="222" rx="15" ry="10" fill="url(#cheek)" />
        <ellipse cx="245" cy="222" rx="15" ry="10" fill="url(#cheek)" />
        {/* Ears */}
        <ellipse cx="124" cy="200" rx="10" ry="16" fill="url(#skin)" stroke="#1f1712" strokeWidth="4" />
        <ellipse cx="276" cy="200" rx="10" ry="16" fill="url(#skin)" stroke="#1f1712" strokeWidth="4" />
        {/* Eyebrows — bushy */}
        ...
        {/* Mouth */}
        {mouth}
        {/* Mustache — small classic */}
        <path d="M 178 218 Q 188 215 198 220 Q 208 215 222 218 Q 218 226 200 226 Q 184 226 178 218 Z"
              fill="#241409" stroke="#000" strokeWidth="1.5" />

        {/* Hachimaki — red headband with white circle and kanji-ish mark */}
        <g>
          <path d="M 122 158 Q 200 138 278 158 L 282 178 Q 200 158 118 178 Z"
                fill="#d8453d" stroke="#1f1712" strokeWidth="4" strokeLinejoin="round" />
          <circle cx="200" cy="160" r="13" fill="#f6ecd6" stroke="#1f1712" strokeWidth="3" />
          {/* Stylized brush stroke inside circle */}
          <path d="M 196 154 L 196 166 M 191 158 L 204 158 M 200 158 L 205 165"
                stroke="#1f1712" strokeWidth="2.5" strokeLinecap="round" />
          {/* Headband tail flapping right */}
          <path d="M 270 168 Q 305 175 322 200 Q 308 196 296 198 Q 292 188 270 188 Z"
                fill="#d8453d" stroke="#1f1712" strokeWidth="3.5" strokeLinejoin="round" />
        </g>

        {/* Hair tufts */}
        <path d="M 130 140 Q 150 110 200 100 Q 250 110 270 140 Q 248 125 200 124 Q 152 125 130 140 Z"
              fill="#1f1410" stroke="#000" strokeWidth="2" />
      </g>
    </svg>
  );
}

window.Sensei = Sensei;
